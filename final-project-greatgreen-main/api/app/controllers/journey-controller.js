...
export const patch = async (request, response) => {
  try{
    ...
    if((journeyData.status == JOURNEY_STATUS.IN_PROGRESS) && (journey.milestones.length < MIN_MILESTONES_TO_COMPLETE)) {
      ValidationError(`Cannot complete until min ${MIN_MILESTONES_TO_COMPLETE} milestones are added !`);
    }


    /** Remove if any errors found */
    if((journeyData.status == JOURNEY_STATUS.PICKUP_READY)  && (journey.provider_type == CENTER_TYPE.AFFILIATION)) {
